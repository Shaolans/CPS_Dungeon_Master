...
public class Player extends Entity implements PlayerService {
	...
	public boolean isViewable(int col, int row) {
		...
		//System.out.println("ICI :"+this.col + " "+ this.row);

		if(!(col+this.col < env.getWidth() && col+this.col >= 0 && row+this.row >=0 && row+this.row < env.getHeight())) {
			return true;
		}
		...
		if(col == -1 && row == 2){
			if(!notviewable.contains(getNature(-1, 1)) && isViewable(-1, 1)){
				return true;
			}
			return false;
		}
		if(col == 0 && row == 2){
			...
			if(!notviewable.contains(getNature(0, 1)) && isViewable(0, 1)){
				return true;
			}
			return false;
		}
		...
		if(col == -1 && row == 3){
			if(!notviewable.contains(getNature(-1, 2)) && isViewable(-1, 2)){
				return true;
			}
			return false;
		}
	...
	// Vérifier que chaque forward, backaward,.... col+1 dans cadre
